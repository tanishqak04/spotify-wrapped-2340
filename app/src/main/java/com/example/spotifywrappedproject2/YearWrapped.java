...
public class YearWrapped extends AppCompatActivity {
    private API api;
                ...
                db.collection("users").document(userID).collection("wrapped")
                        ...
                        .addOnSuccessListener(new OnSuccessListener<DocumentReference>() {
                            ...
                            public void onSuccess(DocumentReference documentReference) {
                                Toast.makeText(YearWrapped.this, "Saved!", Toast.LENGTH_SHORT).show();
                            }
                        })
                        .addOnFailureListener(new OnFailureListener() {
                            @Override
                            public void onFailure(@NonNull Exception e) {
                                Toast.makeText(YearWrapped.this, "Failed to Save", Toast.LENGTH_SHORT).show();
                            }
                        });
    ...
    private void fetchAndDisplayTopTracks() {
        ...
        api.getTopTracks("long_term", 5, new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                runOnUiThread(() -> Toast.makeText(YearWrapped.this, "Network error: " + e.getMessage(), Toast.LENGTH_SHORT).show());
            }

            @Override
            public void onResponse(Call call, Response response) throws IOException {
                if (!response.isSuccessful()) {
                    runOnUiThread(() -> Toast.makeText(YearWrapped.this, "Error fetching top tracks: " + response.message(), Toast.LENGTH_SHORT).show());
                    return;
                }
            ...
            runOnUiThread(() -> {
                try {
                    for (int i = 0; i < items.length() && i < imageViews.length; i++) {
                        ...
                        Glide.with(YearWrapped.this).load(imageUrl).into(imageViews[i]);
                        textViews[i].setText(trackName);
                    }
                } catch (JSONException e) {
                    Toast.makeText(YearWrapped.this, "JSON parsing error: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                }
            });
        } catch (JSONException e) {
            runOnUiThread(() -> Toast.makeText(YearWrapped.this, "JSON parsing error: " + e.getMessage(), Toast.LENGTH_SHORT).show());
        }
    }
}
