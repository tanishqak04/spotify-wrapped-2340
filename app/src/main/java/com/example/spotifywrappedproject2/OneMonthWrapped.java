...
public class OneMonthWrapped extends AppCompatActivity {
    private API api;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_one_month_wrapped);
                ...
                db.collection("users").document(userID).collection("wrapped")
                        ...
                        .addOnSuccessListener(new OnSuccessListener<DocumentReference>() {
                            ...
                            public void onSuccess(DocumentReference documentReference) {
                                Toast.makeText(OneMonthWrapped.this, "Saved!", Toast.LENGTH_SHORT).show();
                            }
                        })
                        .addOnFailureListener(new OnFailureListener() {
                            @Override
                            public void onFailure(@NonNull Exception e) {
                                Toast.makeText(OneMonthWrapped.this, "Failed to Save", Toast.LENGTH_SHORT).show();
                            }
                        });
    ...
    private void fetchAndDisplayTopTracks() {
        ...
        api.getTopTracks("medium_term", 5, new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                runOnUiThread(() -> Toast.makeText(OneMonthWrapped.this, "Network error: " + e.getMessage(), Toast.LENGTH_SHORT).show());
            }

            @Override
            public void onResponse(Call call, Response response) throws IOException {
                if (!response.isSuccessful()) {
                    runOnUiThread(() -> Toast.makeText(OneMonthWrapped.this, "Error fetching top tracks: " + response.message(), Toast.LENGTH_SHORT).show());
                    return;
                }
            ...
            runOnUiThread(() -> {
                try {
                    for (int i = 0; i < items.length() && i < imageViews.length; i++) {
                        ...
                        Glide.with(OneMonthWrapped.this).load(imageUrl).into(imageViews[i]);
                        textViews[i].setText(trackName);
                    }
                } catch (JSONException e) {
                    Toast.makeText(OneMonthWrapped.this, "JSON parsing error: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                }
            });
        } catch (JSONException e) {
            runOnUiThread(() -> Toast.makeText(OneMonthWrapped.this, "JSON parsing error: " + e.getMessage(), Toast.LENGTH_SHORT).show());
        }
    }
}